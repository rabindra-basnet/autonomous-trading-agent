...
class SentimentFeatures:
    ...
    def compute_sentiment_features(
        ...
        relevant_news = [
            n for n in news if (symbol in n.symbols_mentioned or not n.symbols_mentioned) and (n.published_at >= cutoff)
        ]

        # Filter relevant social
        relevant_social = [s for s in social if s.symbol == symbol and s.timestamp >= cutoff]
        ...
        # Social metrics
        social_sentiment = float(np.mean([s.average_sentiment for s in relevant_social])) if relevant_social else 0.0
        social_mentions = float(sum(s.mention_count for s in relevant_social))
        social_velocity = float(np.mean([s.mention_velocity_pct for s in relevant_social])) if relevant_social else 0.0
